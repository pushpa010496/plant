...
if($wordcount>=2){
   ...
   $products= Product::where('title','LIKE','%'.$request->q.'%')->get();
            ...
            if($request->country != '' && $request->searchtype == 'product'){
             $products = $products->where('company.country',$request->country ); 
           }
...
if($request->searchtype == 'product'){
  ...
  return view('search.searchlanding',compact('products','companies','keyword','country_list','company_filter','category_list'));
    ...
    public function homesearchh(Request $request)
    {
...
if($wordcount>=2){
 ...
 //$products = Product::where('title',$request->q)->get();
	$products= Product::where('title','LIKE','%'.$request->q.'%')->get();
}
else{
 ...
 $products= Product::where('title','LIKE','%'.$request->q.'%')->get();
       ...
       $keyword = $request->q;
         ...
         if($request->q){
            ...
            if($company_ls){
              $company_ls_pro = Product::with('company','compprofile')->whereIn('company_id',$company_ls)->pluck('id');
            ...
            // }
               ...
               // $products = Product::whereIn('id',$products_ids)->where('company_id','!=',null)->where('active_flag',1)->get();
...
if($request->searchtype == 'product'){
 ...
 // return view('search.search_supplier',compact('keyword','country'));
}else{
  return view('search.searchlanding',compact('products','keyword','country_list','company_filter','category_list'));
}
...
public function bySupplier($searchkey){  
  ...
  if($data){
    ...
    { 
      ...
      if(!$category){
        $statusFlag = 'nocategory';
        return view('search.category_search',compact('data','country','category','statusFlag')); 
      }
